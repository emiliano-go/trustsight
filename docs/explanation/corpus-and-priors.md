...
Two AUR-derived artifacts are built from the same mirror: the benchmark **corpus** used to calibrate fire rates, and the novelty **seed** used as a prior. Both start from a bare clone of the AUR monorepo.

### Corpus build

1. **Mirror pull**: a bare clone of `https://github.com/archlinux/aur.git` provides one branch per package (about 116,000 branches).
2. **Diff extraction**: `scripts/build_corpus.py` selects packages by stratum and writes a diff per commit pair, including `.install`, `.patch` and systemd service files where they change.
3. **Pinning**: the selected commit pairs are recorded in `corpus.lock`, so the corpus can be regenerated byte-for-byte.

### Seed build

1. **Observation extraction**: `scripts/generate_seed.py` walks every branch and reads the `.SRCINFO` and `PKGBUILD` blobs at the branch tip, extracting source URLs, maintainer identities and dependency names.
...
The stored snapshot is an optional prior. `trustsight full-aur` is an optional
