...
The tokenizer is the second parser eating hostile input, and it is the one with an amplification property the regex engine does not have. [A6](../security.md#the-invariants) describes it: `b=$a$a` doubles per level, a chain grows as `2**depth`, and a 517-byte PKGBUILD was once enough to OOM the process. Four bounds now hold it (`_MAX_EXPANSION_PASSES`, `_MAX_VALUE_LEN`, `_MAX_LINE_LEN`, `_MAX_TABLE_BYTES`), and they work. But "we found the bomb, then bounded it" is a different assurance from "a defect here cannot reach anything", and the gap between those two is what isolation would close.
