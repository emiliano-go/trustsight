...
| `--repo` | `str` | - | Scan packages from a specific local repository. Can be repeated (`--repo aur --repo testing`). Repo packages are compared against the repo's own advertised version; their names are not sent to the AUR. |
| `--foreign` | flag | `false` | Also include foreign packages (`pacman -Qm`). When used with `--repo`, foreign packages are added to the set. Only foreign package names are ever looked up on the AUR. |
| `--all-repos` | flag | `false` | Automatically detect all local repositories from `/etc/pacman.conf` (excludes official repos: `core`, `extra`, `community`, `multilib`, `testing`, etc.) and scan packages from all of them. Auto-detected repos are compared locally, so a private repository's package names never leave the machine. |
...
2. Compares each package against the right source. A package installed from a local repo is compared against that repo's own advertised version (`pacman -Sl`), which never leaves the machine. Only foreign (AUR) packages are looked up in the AUR metadata snapshot (`full-aur-meta.json`, an offline copy of the AUR package database). On the first run the snapshot is downloaded and used in the same invocation. Later runs reuse it until it is older than `[discovery] metadata_ttl_minutes` (default 60), then refetch it: the version a review compares against is the snapshot's, so a snapshot left to age reports a machine with pending updates as fully current. A refresh that cannot reach the AUR keeps the snapshot on disk and warns that a package updated since then will not be reported.
3. Filters to packages whose installed version is older than the resolved version (using `vercmp`).
