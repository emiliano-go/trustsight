...
All directories and default files are created on first run by `ensure_default_configs()` in `src/trustsight/config.py:207`.
...
URLs in the diff are classified by `classify_url()` in `src/trustsight/buckets.py:22`.
...
| Key | Type | Default | Effect |
|-----|------|---------|--------|
| `url_first_in_package` | int | `10` | Raw weight for a URL never seen before in this package's history. |
| `url_first_globally` | int | `15` | Raw weight for a URL never seen before in any package in the corpus. |
| `maintainer_first_in_package` | int | `20` | Raw weight for a maintainer never seen before for this package. |
...
Pinning classification via `classify_pinning_level()` in `src/trustsight/buckets.py:77`.
...
Read at `src/trustsight/llm.py:68` and `src/trustsight/llm.py:72`.

---

## Default configuration

The full default config is embedded in `src/trustsight/config.py:8` as `DEFAULT_CONFIG` and written to `~/.config/trustsight/config.toml` on first invocation. Users may edit it freely.
