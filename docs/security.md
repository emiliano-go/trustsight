...
So: an **instrument, not a judge**. TrustSight reads AUR PKGBUILD diffs, applies detection rules that are published in full (every shipped rule, its pattern and its severity, is in the [rules reference](reference/rules.md), and the code-emitted rules are in the open source), and reports what it found and what it could not see. It never decides whether a package is safe. A person does. That distinction - **input, not verdict** - is the foundation this page is built on, in four statements the rest of the document makes precise:
