...
Eight things make a run partial, and all eight are recorded as **coverage gaps** on the result:
...
**The body is one body.** All three JSON paths render through `reporting.report_body`, and every key in `reporting.REPORT_KEYS` is present on every one of them, with the same value for the same result. This closes a real divergence rather than restating an intention: the three paths used to build three dicts with two naming conventions between them (`package` against `package_name`, `score` against `final_score`), and the API's body carried no `findings` at all while its docstring claimed to be what the CLI writes. A consumer could be written against one path and silently miss evidence on another, which is [B2](#b2-an-unflagged-verdict-is-never-issued-for-an-analysis-that-was-incomplete)'s failure one layer up: not content skipped without a gap, but a whole field absent without anything saying so.
...
| `CI installs from the lock` | the CI assumption | `uv sync --frozen` in every workflow, `uv.lock` |
