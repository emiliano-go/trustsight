...
def register_commands(app: typer.Typer):
    ...
    def inspect(
        ...
        if last is not None and whole:
            msg = "--last and --whole are not combined"
            if json_output:
                typer.echo(json.dumps({"error": msg}))
            else:
                _print_colored(msg, "red", stderr=True)
        ...
        try:
            fact = analyze_package(package, depth=depth, record=record, whole=whole)
